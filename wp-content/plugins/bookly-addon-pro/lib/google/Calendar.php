    ...
    public function syncAppointment( Appointment $appointment )
    {
        ...
        try {
            ...
            if ( $appointment->hasGoogleCalendarEvent() ) {
                // Update event.
                $event = $this->client->service()->events->patch( $this->_getCalendarId(), $appointment->getGoogleEventId(), $event );
            } else {
                // Create event.
    ...
    protected function _populateEvent( \BooklyGoogle_Service_Calendar_Event $event, Appointment $appointment )
    {
        ...
        if ( $appointment->getCreatedFrom() == 'bookly' ) {
            ...
            if ( $appointment->getServiceId() ) {
                ...
                $service_capacity = 9999;
            }
            $description  = '';
            $client_names = array();

            $event_client_info = get_option( 'bookly_gc_event_client_info' );
            $signed_up         = 0;
            $codes             = null;
            foreach ( $appointment->getCustomerAppointments() as $ca ) {
                $order = Order::create( Customer::find( $ca->getCustomerId() ) );
                if ( $ca->getPaymentId() ) {
                    $order->setPayment( Payment::find( $ca->getPaymentId() ) );
                }
                $item  = Simple::create( $ca )
                    ->setAppointment( $appointment )
                    ->setService( $service );
                $codes = new BooklyLib\Notifications\Assets\Item\Codes( $order );
                $codes->prepareForItem( $item, 'staff' );
                $description .= $codes->replace( $event_client_info ) . PHP_EOL;
                $signed_up   += $ca->getNumberOfPersons();
                $client_names[] = $ca->customer->getFullName();
                if ( $service_capacity == 0 ) {
                    $result = BooklyLib\Entities\StaffService::query()
                        ->select( 'capacity_max' )
                        ->where( 'staff_id',   $item->getStaff()->getId() )
                        ->where( 'service_id', $service->getId() )
                        ->fetchRow();
                    if ( $result ) {
                        $service_capacity = $result['capacity_max'];
                    }
                }
            }

            $appointment_info = strtr( get_option( 'bookly_gc_event_appointment_info' ), array(
                '{signed_up}'         => $signed_up,
                '{number_of_persons}' => $signed_up,
                '{service_capacity}'  => $service_capacity,
                '{client_names}'      => implode( ', ', $client_names ),
                '{on_waiting_list}'   => BooklyLib\Config::waitingListActive() ?
                    CustomerAppointment::query( 'ca' )->where( 'ca.appointment_id', $appointment->getId() )->where( 'status', CustomerAppointment::STATUS_WAITLISTED )->count() : 0,
            ) );
            if ( $codes == null ) {
                $order = Order::create( new Customer() );
                $item  = Simple::create( new BooklyLib\Entities\CustomerAppointment() )
                    ->setAppointment( $appointment )
                    ->setService( $service );
                $codes = new BooklyLib\Notifications\Assets\Item\Codes( $order );
                $codes->prepareForItem( $item, 'staff' );
            ...
            $description = trim( $codes->replace( $appointment_info ) . PHP_EOL . $description );
